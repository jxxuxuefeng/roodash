...
describe('formatData function', () => {
  ...
  it('不传入映射关系，拿默认映射关系，返回和传入的数据保持一致', () => {
    ...
    const newValue = formatData<typeof value, Value>(value);
  ...
  it('传入正确格式的映射关系，返回映射关系后的数据格式', () => {
    // type NewValue = { userName: string; value: number };
    const value = { name: '张三', age: 18 };
  ...
  it('传入不正确格式的映射关系，只映射存在value中的key的关系', () => {
    const value = { name: '张三', age: 18 };
    const newValue = formatData<typeof value, { name1: string; value: number }>(value, {
      map: { name1: 'userName', age: 'value' },
    });
